...
class SunCalc {
    ...
    function getMoonPosition($date) {
        $lw  = rad * -$this->lng;
        $phi = rad * $this->lat;
        $date->getTimestamp() / daySec - 0.5 + J1970;
    ...
    function getMoonIllumination() {
        ...
        if ($phase == 0)
          {$the_phase = "Luna nuova";}
        else if ($phase > 0 && $phase < 0.5)
          { $the_phase = "Luna crescente";}
        else if ($phase == 0.5)
          { $the_phase = "Luna piena";}
        elseif ($phase > 0.5)
        { $the_phase = "Luna decrescente";}

        return $the_phase. ". Fase ".round($fraction*100) ." %";
...
$test = new SunCalc(new DateTime(), 40.60, 14.35);
