...
int wind_dir_degr[]= {0, 23, 45, 68, 90, 113, 135, 158, 180, 203, 225, 248, 270, 293, 315, 338};
...
static int fineoffset_WH24_callback(r_device *decoder, bitbuffer_t *bitbuffer)
{
    ...
    int wind_dir_index = 0;
    while (wind_dir_index < 16 && wind_dir_degr[wind_dir_index] < wind_dir) ++wind_dir_index;
    ...
    /*FILE *fpWH4000 = fopen("/dev/shm/wh4000-rtl_433.txt", "w");
    if(fpWH4000 != NULL){
	void *json_output = data_output_json_create(fpWH4000);
        data_output_print(json_output, data); fprintf(fpWH4000, "\n");
...
/*    if(temp_raw != 0x7ff && humidity != 0xff && wind_dir != 0x1ff && wind_speed_raw != 0x1ff && gust_speed_raw != 0xff && uv_raw != 0xffff && light_raw != 0xffffff){
		time_t timer;
    	char buffer[26];
    	struct tm* tm_info;
    	...
    	FILE *fpWH4000 = fopen("/dev/shm/wh4000-rtl_433.txt", "w+");
		//fpWH4000=fopen("/dev/shm/wh4000-rtl_433.txt", "wb");
    	//fpWH4000=fopen("/dev/shm/wh4000-rtl_433.txt", "w+");
    	if(fpWH4000 != NULL){
        	fprintf(fpWH4000, "{\"time\" : \"%s\",", buffer);
        	fprintf(fpWH4000, "\"model\" : \"Fineoffset-WH4000\","); 
        	fprintf(fpWH4000, "\"id\" : %d,", id);
        	fprintf(fpWH4000, "\"battery\" : \"%s\",", low_battery ? "LOW" : "OK");
        	fprintf(fpWH4000, "\"temperature_C\" : %.01f,", temperature);
        	fprintf(fpWH4000, "\"humidity\" : %u,", humidity);//%u
        	fprintf(fpWH4000, "\"wind_dir_str\" : \"%s\",", wind_dir_string[wind_dir_index]);
        	fprintf(fpWH4000, "\"wind_dir_deg\" : %d,", wind_dir);
        	fprintf(fpWH4000, "\"wind_avg\" : %.1f,", wind_speed_raw * 3.6f);
        	fprintf(fpWH4000, "\"wind_gust\" : %.1f,", gust_speed_raw * 3.6f);
        	//fprintf(fpWH4000, "\"wind_avg\" : %d,", wind_speed_raw);
        	//fprintf(fpWH4000, "\"wind_gust\" : %d,", gust_speed_raw);
        	fprintf(fpWH4000, "\"rain_mm\" : %.1f,", rainfall_mm);
        	fprintf(fpWH4000, "\"uv\" : %d,", uv_raw);
        	fprintf(fpWH4000, "\"uvi\" : %d,", uv_index);
        	fprintf(fpWH4000, "\"light_lux\" : %.1f,", light_lux);
        	fprintf(fpWH4000, "\"mic\" : \"CRC\"}\n");
			printf("fpWH4000 writed");
    	}else{
        	printf("Error %d \n", errno);
    	}
    	fclose(fpWH4000);
