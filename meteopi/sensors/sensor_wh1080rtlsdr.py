...
def log(message) :
    print (datetime.datetime.now().strftime("[%d/%m/%Y-%H:%M:%S] [WH1080RTLSDR_] -") , message)
...
class Sensor_WH1080RTLSDR(sensors.sensor_external.Sensor):
    ...
    def ReadData(self):
        with open('/dev/shm/wh1080-rtl_433.txt') as f:
            #data_file = open('/dev/shm/wh1080-rtl_433.txt','r')
            data_file = f.read()
            try:
                ...
                if str(line['msg_type']) == '0':
                    ...
                    if str(self.cfg.rtlsdr_timesync) == 'True':                     
                        ...
                        log("rtlsdr_timesync is disabled.")
                        return "Time",0,0,0,0,"",0,0
            except:
