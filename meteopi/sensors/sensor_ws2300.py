...
import serial
import string
import sensors.ws2300
import serial


def log(message) :
    print (datetime.datetime.now().strftime("[%d/%m/%Y-%H:%M:%S] -[WS23XX_______] -") , message)
...
class Sensor_WS2300(sensors.sensor_external.Sensor):
    '''
    ...
    def GetData(self):
        ...
        #print "GetData"
        serialPort = sensors.ws2300.LinuxSerialPort(self.cfg.sensor_serial_port)
        #print serialPort
        #print "opened"

        ws = sensors.ws2300.Ws2300(serialPort)

        measures = [
            sensors.ws2300.Measure.IDS["pa"],  # pressure absolute
            sensors.ws2300.Measure.IDS["it"],  # in temp
            sensors.ws2300.Measure.IDS["ih"],  # in humidity
            sensors.ws2300.Measure.IDS["ot"],  # out temp"
            sensors.ws2300.Measure.IDS["oh"],  # out humidity"
            sensors.ws2300.Measure.IDS["rt"],  # rain total
            sensors.ws2300.Measure.IDS["ws"],  # "wind speed"
            sensors.ws2300.Measure.IDS["w0"],  # "wind direction"
            sensors.ws2300.Measure.IDS["ws"],  # "wind speed gust ???"
            sensors.ws2300.Measure.IDS["wsu"],  # wind speed units
        ...
        while True:
            ...
            try:
                ...
                raw_data = sensors.ws2300.read_measurements(ws, measures)
