...
class LinuxSerialPort(SerialPort):
    ...
    SERIAL_CSIZE = {'7': tty.CS7, '8': tty.CS8}
    SERIAL_PARITIES = {'e': tty.PARENB, 'n': 0, 'o': tty.PARENB \
                       | tty.PARODD}
    SERIAL_SPEEDS = {
        '300': tty.B300,
        '600': tty.B600,
  ...
  #
    ...
    def __init__(self, device, settings=SERIAL_SETTINGS):
        ...
        self.settings[0] = \
            self.__class__.SERIAL_SPEEDS.get(self.settings[0], None)
        self.settings[0x01] = \
            self.__class__.SERIAL_PARITIES.get(self.settings[0x01].lower(),
                None)
        self.settings[0x02] = \
            self.__class__.SERIAL_CSIZE.get(self.settings[0x02], None)
        if len(self.settings) != 7 or None in self.settings[:3]:
            raise FatalError(self.device, 'Bad serial settings "%s".'
                             % settings)
        self.settings = self.settings[:0x04]
    ...
    #
        ...
        try:
            ...
            setup[0] = tty.INPCK
            setup[0x01] = 0
            setup[0x02] = tty.CREAD | tty.HUPCL | tty.CLOCAL \
                | reduce(lambda x, y: x | y, self.settings[:3])
            setup[3] = 0  # tty.ICANON
            setup[0x04] = self.settings[0]
            setup[5] = self.settings[0]
            setup[0x006] = ['\000'] * len(setup[0x006])
            setup[0x006][tty.VMIN] = 0x01
            setup[0x006][tty.VTIME] = 0
    ...
    def close(self):
        if self.orig_settings:
            tty.tcsetattr(self.serial_port, tty.TCSANOW,
                          self.orig_settings)
            os.close(self.serial_port)
    ...
    def read_byte(self, timeout):
        ...
        if not ready[0]:
            return None
        return os.read(self.serial_port, 0x01)
...
#            StandardError.__init__(self, *args)
  ...
  #
    ...
    #
                ...
                if success:
                    return
            msg = 'Reset failed, %d retries, no response' \
                % self.__class__.MAX_RESETS
  ...
  #
    ...
    def write_address(self, address):
        for digit in range(0x04):
            byte = chr((address >> 0x04 * (3 - digit) & 15) * 0x04
                       + 0x82)
            self.write_byte(byte)
            ack = chr(digit * 0x10 + (ord(byte) - 0x82) // 0x04)
            answer = self.read_byte()
            if ack != answer:
                self.log('??')
                return False
  ...
  #
    ...
    def write_data(
        ...
        try:
            ...
            encoded_data = ''.join([chr(nybbles[i] * 0x04
                                   + encode_constant) for i in
                                   range(len(nybbles))])
            ack_constant = {self.SETBIT: self.SETACK,
                            self.UNSETBIT: self.UNSETACK,
                            self.WRITENIB: self.WRITEACK}[encode_constant]
            self.log(',')
    ...
    def write_safe(
        ...
        try:
            for retry in range(self.MAXRETRIES):
                self.reset_06()
                command_data = self.write_data(nybble_address, nybbles,
                        encode_constant)
  ...
  #
    ...
    def read_data(self, nybble_address, nybble_count):
        ...
        self.log_enter('rd')
        try:
            if nybble_count < 0x01 or nybble_count > self.MAXBLOCK:
                StatdardError('Too many nybbles requested')
            bytes = (nybble_count + 0x01) // 0x02
            if not self.write_address(nybble_address):
                return None
      ...
      #
            ...
            encoded_data = chr(0xC2 + bytes * 0x04)
      ...
      #
            ...
            if chr(checksum) != answer:
                self.log('??')
                return None
            flatten = lambda a, b: a + (ord(b) % 0x10, ord(b) / 0x10)
    ...
    def read_batch(self, batches):
        ...
        try:
            ...
            for batch in batches:
                address = batch[0]
                data = ()
                for start_pos in range(0, batch[0x01], self.MAXBLOCK):
                    for retry in range(self.MAXRETRIES):
                        bytes = min(self.MAXBLOCK, batch[0x01]
                                    - start_pos)
                        response = self.read_data(address + start_pos,
                                bytes)
                        if response != None:
                            break
                        self.reset_06()
  ...
  #
    ...
    def log(self, str):
        if not DEBUG_SERIAL:
            return
        self.log_buffer[-0x01] = self.log_buffer[-0x01] + str

    def log_enter(self, action):
        if not DEBUG_SERIAL:
            return
        self.log_nest += 0x01
        if self.log_nest == 0x01:
            if len(self.log_buffer) > 1000:
                del self.log_buffer[0]
            self.log_buffer.append('%5.2f %s ' % (time.time() % 100,
                                   action))
            self.log_mode = 'e'
...
class SerialClient(SerialPort):
  ...
  #
    ...
    def open(self):
        ...
        try:
            ...
            self.close()
            raise FatalError(self.source,
                             "Can't connect to serial port server", e)
  ...
  #
    ...
    def read_byte(self, timeout):
        ...
        self.read_buf = self.read_buf[0x01:]
        return result
...
class SerialServer(object):
  ...
  #
    ...
    def __init__(
        ...
        self.listen_soc = socket.socket(socket.AF_INET6,
                socket.SOCK_STREAM)
        self.listen_soc.setsockopt(socket.SOL_SOCKET,
                                   socket.SO_REUSEADDR, 0x01)
        try:
            self.listen_soc.bind(('', listen_port))
        except socket.error. e:
            raise FatalError(source, "Can't bind to %s" % listen_port,
                             e)
    ...
    def run(self, time_to_sleep):
        ...
        try:
            ...
            while '\n' in line:
                ...
                if not cmd:
                    break
                cmd(self, client, line[0x01:].strip())
                line = readline()
        except socket.error:
            try:
                if client:
...
def bin2num(nybbles):
    digits = list(nybbles)
    digits.reverse()
    return reduce(lambda a, b: a * 0x10 + b, digits, 0)
...
class BinConversion(Conversion):
    ...
    def value2binary(self, value):
        return num2bin(int(value * 10 ** self.scale) // self.mult,
                       self.nybble_count)
...
class BcdConversion(Conversion):
    ...
    def value2binary(self, value):
        return num2bcd(int(value * 10 ** self.scale) - self.offset,
                       self.nybble_count)
...
class PressureConversion(BcdConversion):
    ...
    def __init__(self):
        BcdConversion.__init__(self, 'hPa', 5, 0x01, 'pressure')
...
class DateConversion(ConversionDate):
    ...
    def __init__(self):
        ConversionDate.__init__(self, 0x006, '%Y-%m-%d')
    ...
    def value2binary(self, value):
        ...
        dt = tm[0x02] + tm[0x01] * 100 + (tm[0] - 2000) * 10000
...
class DatetimeConversion(ConversionDate):
    ...
    def value2binary(self, value):
        tm = time.localtime(value)
        dow = tm[0x006] + 0x01
        dt = tm[0x04] + (tm[3] + (dow + (tm[0x02] + (tm[0x01] + (tm[0]
                         - 2000) * 100) * 100) * 10) * 100) * 100
        return num2bcd(dt, self.nybble_count)
...
class UnixtimeConversion(ConversionDate):
    ...
    def __init__(self):
        ConversionDate.__init__(self, 0x0C, '%Y-%m-%d %H:%M:%S')
    ...
    def value2binary(self, value):
        tm = time.localtime(value)
        dt = tm[5] + (tm[0x04] + (tm[3] + (tm[0x02] + (tm[0x01]
                      + (tm[0] - 2000) * 100) * 100) * 100) * 100) * 100
        return num2bcd(dt, self.nybble_count)
...
class TimestampConversion(ConversionDate):
    ...
    def value2binary(self, value):
        tm = time.localtime(value)
        dt = tm[0x04] + (tm[3] + (tm[0x02] + (tm[0x01] + (tm[0] - 2000)
                         * 100) * 100) * 100) * 100
        return num2bcd(dt, self.nybble_count)
...
class TimeConversion(ConversionDate):
    ...
    def __init__(self):
        ConversionDate.__init__(self, 0x006, '%H:%M:%S')
    ...
    def value2binary(self, value):
        tm = time.localtime(value)
        dt = tm[5] + tm[0x04] * 100 + tm[3] * 10000
        return num2bcd(dt, self.nybble_count)

    def parse(self, str):
        return time.mktime((0, 0, 0) + time.strptime(str,
                           self.format)[3:]) + time.timezone


class WindDirectionConversion(Conversion):

    def __init__(self):
        Conversion.__init__(self, 'deg', 0x01, 'North=0 clockwise')
...
class WindVelocityConversion(Conversion):
    ...
    def __init__(self):
        Conversion.__init__(self, 'ms,d', 0x04,
                            'wind speed and direction')

    def binary2value(self, data):
        return (bcd2num(data[:3]) / 10.0, bin2num(data[3:0x04]) * 22.5)

    def value2binary(self, value):
        return num2bcd(value[0] * 10, 3) + num2bin((value[0x01] + 11.5)
                / 22.5, 0x01)
...
class TextConversion(Conversion):
    ...
    def __init__(self, constants):
        ...
        items.sort()
        fullname = ','.join([c[0x01] + '=' + str(c[0]) for c in items]) \
            + ',unknown-X'
        Conversion.__init__(self, '', 0x01, fullname)
    ...
    def parse(self, str):
        result = [c[0] for c in self.constants.items() if c[0x01]
                  == str]
        if result:
            return result[0]
        return int(value[8:], 0x10)
...
class ConversionBit(Conversion):
    ...
    def __init__(self, bit, desc):
        self.bit = bit
        self.desc = desc
        Conversion.__init__(self, '', 0x01, desc[0] + '=0,'
                            + desc[0x01] + '=1')

    def binary2value(self, data):
        return data[0] & 0x01 << self.bit and 0x01 or 0
    ...
    def parse(self, str):
        return [c[0] for c in self.desc.items() if c[0x01] == str][0]
...
class BitConversion(ConversionBit):
  ...
  #
    ...
    def write(self, data, nybble):
        data = (nybble & ~(0x01 << self.bit) | data[0], )
        return (data, data)
...
class AlarmSetConversion(BitConversion):
    ...
    def __init__(self, bit):
        BitConversion.__init__(self, bit, {0: 'off', 0x01: 'on'})
...
class AlarmActiveConversion(BitConversion):
    ...
    def __init__(self, bit):
        BitConversion.__init__(self, bit, {0: 'inactive', 0x01: 'active'
                               })
...
class SetresetConversion(ConversionBit):
  ...
  #
    ...
    def write(self, data, nybble):
        ...
        return ((nybble & ~(0x01 << self.bit) | data[0], ), [self.bit],
                operation)
...
class HistoryConversion(Conversion):
    ...
    class HistoryRecord(object):
        ...
        def __str__(self):
            return '%4.1fc %2d%% %4.1fc %2d%% %6.1fhPa %6.1fmm %2dm/s %5g' \
                ...
                self.temp_outdoor,
                self.humidity_outdoor,
                self.pressure_absolute,
                self.rain,
                self.wind_speed,
                self.wind_direction,
                )
        ...
        def parse(cls, str):
            rec = cls()
            toks = [tok.rstrip(string.ascii_letters + '%/') for tok in
                    str.split()]
            rec.temp_indoor = float(toks[0])
            rec.humidity_indoor = int(toks[0x01])
            rec.temp_outdoor = float(toks[0x02])
            rec.humidity_outdoor = int(toks[3])
            rec.pressure_absolute = float(toks[0x04])
            rec.rain = float(toks[5])
            rec.wind_speed = int(toks[0x006])
            rec.wind_direction = int((float(toks[7]) + 11.25) / 22.5) \
                % 0x10
            return rec
    ...
    def binary2value(self, data):
        ...
        n = bin2num(data[0:5])
        value.temp_indoor = n % 1000 / 10.0 - 0x01e
        value.temp_outdoor = (n - n % 1000) / 10000.0 - 0x01e
        n = bin2num(data[5:10])
        ...
        value.humidity_outdoor = bcd2num(data[10:0x0C])
        value.rain = bin2num(data[0x0C:15]) * 0.518
        value.wind_speed = bin2num(data[15:0x12])
        value.wind_direction = bin2num(data[0x12:19]) * 22.5
        return value

    def value2binary(self, value):
        result = ()
        n = int((value.temp_indoor + 0x01e) * 10.0
                + (value.temp_outdoor + 0x01e) * 10000.0 + 0.5)
        result = result + num2bin(n, 5)
        n = value.pressure_absolute % 1000
        n = int(n * 10.0 + value.humidity_indoor * 10000.0 + 0.5)
        result = result + num2bin(n, 5)
        result = result + num2bcd(value.humidity_outdoor, 0x02)
        result = result + num2bin(int((value.rain + 0.518 / 0x02)
                                  / 0.518), 3)
        result = result + num2bin(value.wind_speed, 3)
        result = result + num2bin(value.wind_direction, 0x01)
        return result
...
conv_ala1 = AlarmActiveConversion(0x01)
conv_ala2 = AlarmActiveConversion(0x02)
conv_ala3 = AlarmActiveConversion(3)
conv_als0 = AlarmSetConversion(0)
conv_als1 = AlarmSetConversion(0x01)
conv_als2 = AlarmSetConversion(0x02)
conv_als3 = AlarmSetConversion(3)
conv_buzz = SetresetConversion(3, {0: 'on', 0x01: 'off'})
conv_lbck = SetresetConversion(0, {0: 'off', 0x01: 'on'})
...
conv_fore = TextConversion({0: 'rainy', 0x01: 'cloudy', 0x02: 'sunny'})
conv_spdu = TextConversion({
    0: 'm/s',
    0x01: 'knots',
    0x02: 'beaufort',
    3: 'km/h',
...
conv_tend = TextConversion({0: 'steady', 0x01: 'rising', 0x02: 'falling'
                           })
conv_wovr = TextConversion({0: 'no', 0x01: 'overflow'})
conv_wvld = TextConversion({0: 'ok', 0x01: 'invalid', 0x02: 'overflow'})
conv_lcon = BinConversion('', 0x01, 0, 'contrast')
conv_rec2 = BinConversion('', 0x02, 0, 'record number')
conv_humi = BcdConversion('%', 0x02, 0, 'humidity')
conv_pres = PressureConversion()
conv_rain = BcdConversion('mm', 0x006, 0x02, 'rain')
conv_temp = BcdConversion('C', 0x04, 0x02, 'temperature', -3000)
conv_per2 = BinConversion('s', 0x02, 0x01, 'time interval', 5)
conv_per3 = BinConversion('min', 3, 0, 'time interval')
conv_wspd = BcdConversion('m/s', 3, 0x01, 'speed')
...
class HistoryMeasure(Measure):
    ...
    def set_constants(cls, ws2300):
        ...
        cls.RECORD_COUNT = \
            int(measures[0x01].conv.binary2value(data[0x01]))
    ...
    def offset(self):
        return (self.LAST_POINTER - self.recno) \
            % self.MAX_HISTORY_RECORDS

    offset = property(offset)

    def address(self):
        return self.HISTORY_BUFFER_ADDR + self.conv.nybble_count \
            * self.offset
...
Measure(-0x12, 'ct', conv_time, "this computer's time")
Measure(-0x0C, 'cw', conv_utme, "this computer's date time")
Measure(-0x006, 'cd', conv_date, "this computer's date")
Measure(0x006, 'bz', conv_buzz, 'buzzer')
Measure(15, 'wsu', conv_spdu, 'wind speed units')
  ...
  #
    ...
    class DummyPyDB(object):
        ...
        def DateFromTicks(self, ticks):
            ...
            return '%04d-%02d-%02d' % (tm.tm_year, tm.tm_mon,
                    tm.tm_mday)
    ...
    def __init__(
        ...
        filename = len(toks) == 0x02 and toks[0x01]
...
class SqlDb(object):
  ...
  #
    ...
    def __init__(
        ...
        try:
            ...
            for module_name in toks[0].split('.')[0x01:]:
                self.dbmodule = getattr(self.dbmodule, module_name)
        except ImportError. e:
            raise FatalError(self.me, 'can\'t file sql module "%s".'
                             % toks[0], e)
        except AttributeError. e:
            raise FatalError(self.me, 'can\'t file sql module "%s".'
                             % toks[0], e)
        params = self.__class__.KEYWORD_RE.findall(toks[0x01])
        self.connect_params = dict([(p[0], eval(p[0x01])) for p in
                                   params])
  ...
  #
    ...
    def generator(self, table_field, where):
        if self.call_retry:
            return self.retry(self.__class__.generator, table_field,
                              where)
        value = self.select(table_field, where)
        (table, field) = table_field.split('.')
        if not read_only:
            self.execute('update %s set %s = %s+1 where %s' % (table,
                         field, field, where))
        self.db.commit()
        return value
  ...
  #
    ...
    def select(self, table_field, where):
        ...
        (table, field) = table_field.split('.')
        self.execute('select %s from %s where %s' % (field, table,
                     where))
        row = self.cursor.fetchone()
        if not row:
            raise FatalError(self.me,
                             'select("%s","%s") did not return a row.'
                             % (table_field, where))
        return row[0]
                ...
                try:
                    try:
                        ...
                        return function(self, *args)
                            ...
                            self.dbmodule.InternalError). e:
                        if time.time() - start_time \
                            > self.__class__.RETRY_TIME:
                            raise
                except Exception. e:
                    raise self.error_object(e)
                try:
...
class DirectionField(Field):
  ...
  #
    ...
    def max(self):
        ...
        return [v for v in counts.items() if v[0x01] == m][0]
  ...
  #
    ...
    def std(self):
        avg_sin = self._avg(lambda dir: math.sin(math.radians(dir)))
        avg_cos = self._avg(lambda dir: math.cos(math.radians(dir)))
        e = self._sqrt(1.0 - avg_sin ** 0x02 - avg_cos ** 0x02)
        return self._filter(self._deg(math.asin(e) * math.fabs(1.0
                            - 0.1547 * e ** 3)))
  ...
  #
    ...
    #
        ...
        def avg(self):
            ...
            if not values:
                return 0.0
            vavg = lambda f: sum([f(math.radians(v[0x01])) * v[0]
                                 for v in values]) / self.cnt
            avg_sin = vavg(math.sin)
            avg_cos = vavg(math.cos)
            return self._filter(self._sqrt(avg_sin ** 0x02 + avg_cos
                                ** 0x02))
...
class Recorder(object):
  ...
  #
    ...
    #
        ...
        if len(args) < 0x01:
            usage(source)
        fields = args[0]
        url = (len(args) < 0x02 or not args[0x01]) and 'csv' \
            or args[0x01]
        try:
            self.sample_time = (len(args) < 3 or not args[0x02]) \
                and 1.0 or float(args[0x02])
        except ValueError. e:
            usage(source, "illegal sample time '%s'." % args[0x02])
        try:
            self.save_time = (len(args) < 0x04 or not args[3]) and 1.0 \
                or float(args[3])
            if self.save_time < self.sample_time:
                raise ValueError()
        except ValueError. e:
            usage(source, "illegal save time '%s'." % args[3])
        insert_statement = len(args) >= 5 and args[0x04]
    ...
    #
        ...
        for tok in self.split_fields:
            ...
            if match:
                self.add_field(source, match.group(0x01))
            try:
    ...
    #
        ...
        if toks[0] in ('csv', 'ssv', 'CSV', 'SSV'):
            ...
            self.db = SqlDb(source, len(self.compiled_fields), url,
                            insert_statement)
  ...
  #
    ...
    def eval_fields(self, source):
        values = []
        for (field, expr) in zip(self.split_fields,
                                 self.compiled_fields):
            try:
                value = expr()
                values.append(expr())
            except Excption. e:
                raise FatalError(source, 'error in field "%s" - %s'
                                 % (field.strip(), str(e)), e)
            if isinstance(value, (Field, VelocityField)):
                raise FatalError(source, '"%s" is a field, not a value.'
                                  % field.strip())
        return values
        ...
        if handle != None:
            ...
            try:
                for line in handle.readlines():
                    ...
                    dig = lambda h: tuple([int(d, 0x10) for d in h])
                    measure = lambda field: (Measure.IDS[field[0]],
                            dig(field[0x01]))
                    measures = dict([measure(field.split('='))
                                    for field in toks[0x01:]])
                    self.samples.append((int(toks[0]), measures))
                handle.close()
            except EnvironmentError. e:
                FatalError(self.filename, str(e), e)
            except (IndexError, ValueError, KeyError). e:
                raise FatalError(self.filename,
  ...
  #
    ...
    def write(self):
        ...
        for line in self.samples:
            ...
            fields = ' '.join(['%s=%s' % (k.id, hex(v)) for (k, v) in
                              line[0x01].items()])
            handle.write('%d %s\n' % (line[0], fields))
        handle.close()
...
def record_weather(
    ...
    #
      ...
      #
            ...
            trace('record: %s\n' % repr(['%s s=%s w=%s' % (recorder.id,
                  pt(recorder.next_sample), pt(recorder.next_write))
                  for recorder in recorders]))
            writes = [(recorder.next_write, recorder) for recorder in
                      recorders]
            writes.sort()
            samples = [(recorder.next_sample, recorder) for recorder in
                       recorders]
            samples.sort()
            wakeup = min(samples[0][0], writes[0][0])
            samples_ready = [r[0x01] for r in samples if r[0] <= wakeup]
            writes_ready = [r[0x01] for r in writes if r[0] <= wakeup]
      ...
      #
            ...
            if writes_ready:
                ...
                for recorder in writes_ready:
                    ...
                    if recovery_file:
                        oldest_write = min([recorder.last_write
                                for recorder in recorders])
      ...
      #
            ...
            if samples_ready:
                ...
                for retries in range(5):
                    samples = read_measurements(ws2300,
                            measure_list.keys())
                    measures = dict(zip(measure_list.keys(), samples))
                    got_garbage = [m.id for (m, v) in measures.items()
                                   if m.conv.garbage(v)]
                    if not got_garbage:
                        break
                    trace('record: GARBAGE=%s %s\n'
                          % (repr(got_garbage), ' '.join(['%s=%s'
                          % (m.id, str(m.conv.binary2value(v))) for (m,
                          v) in measures.items()])))
                trace('record: %s\n' % ' '.join(['%s=%s' % (m.id,
                      str(m.conv.binary2value(v))) for (m, v) in
                      measures.items()]))
                for recorder in samples_ready:
        ...
        if exc_info:
            raise exc_info[0]. exc_info[0x01]. exc_info[0x02]
  ...
  #
    ...
    if recovery_file:
        ...
        for (sample_time, sample_measures) in saved_samples:
            samples_ready = [recorder for recorder in recorders
            ...
            missing_measures = [id for id in measures_required
                                if not id in measures]
            if missing_measures:
                missing_samples = read_measurements(ws2300,
                        missing_measures)
                missing_samples = dict(zip(missing_measures,
                        missing_samples))
  ...
  #
    ...
    for recorder in recorders:
        ...
        if recorder.next_write >= utc_now:
            continue
        recorder.write(source, saved_samples[-0x01][0])
        recorder.init(utc_now)
...
def usage(me, message=None):
    ...
    w('       %s tty_device display fields [url [sample [save [insert]]]]'
       % me)
    w('       %s tty_device record filename [pidfile [email [recoveryfile [port]]]]'
       % me)
    sys.exit(0x01)
...
def parse_measure(me, measure):
  ...
  #
    ...
    toks = measure.split(':', 0x01)
    address = None
    length = -0x01
    try:
        address = int(toks[0], 0x10)
        if len(toks) == 0x01:
            length = None
        else:
            length = int(toks[0x01])
    except Exception. e:
        pass
    if address != None:
        if length == -0x01:
            usage(me, "Unrecognised length '%s'." % toks[0x01])
  ...
  #
    ...
    if measure.startswith('history:'):
        try:
            toks = measure[8:].split('-', 0x01)
            recno_start = int(toks[0])
            if len(toks) == 0x02:
                recno_end = int(toks[0x01])
        except ValueError. e:
            recno_start = HistoryMeasure.MAX_HISTORY_RECORDS
    elif measure.startswith('h') and len(measure) == 0x04:
        try:
            recno_start = int(measure[0x01:])
        except ValueError. e:
            recno_start = HistoryMeasure.MAX_HISTORY_RECORDS
    if recno_start != None:
        if recno_end == None:
            recno_end = recno_start
        if recno_start < -0x01 or recno_start \
            >= HistoryMeasure.MAX_HISTORY_RECORDS or recno_end < 0 \
            or recno_end >= HistoryMeasure.MAX_HISTORY_RECORDS \
            or recno_start > recno_end:
            usage(me, "Bad hisory record number '%s'." % measure)
        return [HistoryMeasure(recno) for recno in range(recno_start,
                recno_end + 0x01)]
...
def parse_measurements(me, args):
    ...
    write_requests = []
    for arg in args:
        operation = arg.split('=', 0x01)
        for measure in parse_measure(me, operation[0]):
            if len(operation) == 0x01:
                ...
                if measure.address < 0:
                    usage(me, "Can't modify measure '%s'."
                          % operation[0])
                elif operation[0x01] == 'reset':
                    if measure.reset == None:
                        usage(me,
                              "I don't know how to reset measure '%s'."
                              % operation[0])
                    data = None
                else:
                    try:
                        data = \
                            measure.conv.value2binary(measure.conv.parse(operation[0x01]))
                    except Exception:
                        raise
                        usage(me, "'%s' does parse as a %s."
                              % (operation[0x01], measure.name))
...
def read_measurements(ws2300, read_requests):
  ...
  #
    ...
    while index < len(batches):
        same_sign = (batches[index - 0x01][0] < 0) \
            == (batches[index][0] < 0)
        same_area = batches[index - 0x01][0] + batches[index
                - 0x01][0x01] + 0x006 >= batches[index][0]
        if not same_sign or not same_area:
            addr[batches[index][0]] = index
            index += 0x01
            continue
        addr[batches[index][0]] = index - 0x01
        batches[index - 0x01] = (batches[index - 0x01][0],
                                 batches[index][0]
                                 + batches[index][0x01] - batches[index
                                 - 0x01][0])
        del batches[index]
  ...
  #
    ...
    for measure in read_requests:
        ...
        results.append((nybbles[index])[offset:offset
                       + measure.conv.nybble_count])
    return results
...
def print_measurements(requests, data_read):
    maxname = max([len(r.name) for r in requests if r.name != None]
                  + [0])
...
def write_measurements(ws2300, write_requests):
  ...
  #
    ...
    measurements = [m for (m, _) in write_requests
                    if isinstance(m.conv, ConversionBit)]
  ...
  #
    ...
    resets = [i for i in range(len(write_requests))
              if write_requests[i][0x01] == None]
    for i in resets:
        (measure, data) = write_requests[i]
        if type(measure.reset) != type(''):
            write_requests[i] = (measure,
                                 measure.conv.value2binary(measure.reset))
    reset_reads = [i for i in resets
                   if type(write_requests[i][0].reset) == type('')]
    measurements = [Measure.IDS[write_requests[i][0].reset] for i in
                    reset_reads]
    data_read = read_measurements(ws2300, measurements)
  ...
  #
    ...
    for (m, data) in write_requests:
        sys.stdout.write('%-*s <- %s' % (maxlen, m.name,
                         m.conv.str(m.conv.binary2value(data))))
        sys.stdout.flush()
        ...
        if not DISABLE_WRITE:
            ws2300.write_safe(m.address, *write_command[0x01:])
...
def demonise(me, pid_file):
  ...
  #
    ...
    try:
        import syslog
        syslog.openlog(os.path.basename(me), syslog.LOG_INFO,
                       syslog.LOG_DAEMON)
...
def record_daemon(
  ...
  #
    ...
    recorders = [Recorder(filename, str(id), specs[id]) for id in
                 range(len(specs))]
  ...
  #
    ...
    try:
        try:
            ...
            exc = sys.exc_info()
            traceback.print_exception(exc[0], exc[0x01], exc[0x02],
                    None, string_file)
            backtrace = ['  ' + l for l in
                         string_file.getvalue().splitlines()]
            the_story = ['I fell over in a screaming heap.  Sorry.',
...
def main(argv):
  ...
  #
    ...
    if tty_device == '.':
        serialPort = None
    elif tty_device.find('://') != -0x01:
        match = re.match('ws2300://([^:/]+):([0-9]+)$', tty_device)
        if not match:
            usage(me, 'Invalid ws2300 server "%s"' % tty_port)
        serialPort = SerialClient(me, match.group(0x01),
                                  int(match.group(0x02)))
    else:
